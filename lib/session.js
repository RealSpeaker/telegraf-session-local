...
class LocalSession {
	...
	async saveSession (key, data) {
		...
		// TODO: Fix returning session if storage based on Promise or Sync behavior
		let session = {}
		...
		if (this.DB.get('sessions').getById(key).value()) {
			debug(' -> Updating')
			session = await this.DB.get('sessions').updateById(key, { data: data }).write()
			return session
		}
		// If no, so we should push new record into it
		else {
			debug(' -> Inserting')
			session = await this.DB.get('sessions').push({ id: key, data: data }).write()
			return session[0]
		}
